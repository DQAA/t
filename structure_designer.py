...
class StructDesignerXYZ(AbstractStructureDesigner):
    ...
    def __init__(self, **kwargs):
        ...
        if reader is None:
            ...
            labels, coords = xyz2tensor(reader)
                                                          # add
        try:
            with open(xyz_new, 'r') as read_file:
                reader_new = read_file.read()
        except IOError:
            reader_new = xyz_new

        if reader_new is None:
            labels_new = kwargs.get('labels', None)
            coords_new = kwargs.get('coords', None)
        else:
            labels_new, coords_new = xyz2tensor(reader_new)
        ...
        self._num_of_species_new = count_species(labels_new)             # add
        self._num_of_nodes_new = sum(self._num_of_species_new.values())
        self._atom_list_new = OrderedDict(list(zip(labels_new, coords_new)))
        for key in self._atom_list:
    ...
    def _sort_disposition(self, dp_labels, dp_coords):

        if not isinstance(dp_coords, torch.Tensor):
            dp_coords = torch.tensor(dp_coords, dtype=torch.float64)
        h_matrix = torch.zeros((dp_coords.shape[0], dp_coords.shape[0]))

        _nn_distance = 2 * self._nn_distance
        for j in range(len(dp_coords)):
            ans = self.get_neighbours(j)
            h_matrix[j, ans] = 1

        _nn_distance = _nn_distance / 2

        indices = self.sort_func(coords=dp_coords,
                                 left_lead=self.left_lead,
                                 right_lead=self.right_lead,
                                 mat=h_matrix)
        ...
        self.reorder = indices
        if (isinstance(self.left_lead, list) or isinstance(self.left_lead, np.ndarray)) and len(self.left_lead) > 0:
            left_lead = np.squeeze(np.concatenate([np.where(indices == item) for item in self.left_lead]))
        if (isinstance(self.right_lead, list) or isinstance(self.right_lead, np.ndarray)) and len(self.right_lead) > 0:
            right_lead = np.squeeze(np.concatenate([np.where(indices == item) for item in self.right_lead]))
        dp_coords = dp_coords[indices]
        dp_labels = [dp_labels[i] for i in indices]

        self.dp_atom_list = OrderedDict(list(zip(dp_labels, dp_coords)))
